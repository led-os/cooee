...
public class WallpaperService
{
	...
	public List<WallpaperInformation> queryDownloadList()
	{
		...
		//				result.add( infor );
		//		}
		File file = new File( PathTool.getAppDir() );
		String wallpapers[] = file.list();
		if( wallpapers != null )
		{
			List<String> download = new ArrayList<String>();
			Tools.getThumblist( wallpapers , download );
			for( String item : download )
			{
				WallpaperInformation infor = new WallpaperInformation();
				infor.setWallpaperDownloadItem( item.replace( "_small.jpg" , "" ) );
				result.add( infor );
			}
