...
public class PubProviderHelper
{
			...
			String value )
	{
		try
		{
			...
			Uri uri = null;
			uri = Uri.parse( "content://" + authority + "/" + table );
			//Log.v("TabWallpaperFactory" ,"uri = "+uri);
			String where = " propertyName=? ";
			String[] args = new String[]{ name };
			ContentValues values = new ContentValues();
			values.put( "propertyName" , name );
			//Log.v("TabWallpaperFactory" ,"value = "+value);
			values.put( "propertyValue" , value );
			//Log.v("TabWallpaperFactory" ,"where = "+where+"; values = "+values+"; args = "+args);
			return resolver.update( uri , values , where , args );
		}
		catch( Exception ex )
		{
			Log.v( "TabWallpaperFactory" , "ex = " + ex + "ex: " + ex.getStackTrace() );
			ex.printStackTrace();
			return 0;
		}
	}
			...
			String value )
	{
		try
		{
			...
			String[] args = new String[]{ name };
			Cursor cursor = resolver.query( uri , null , where , args , null );
			//Log.v("TabWallpaperFactory" ,"cursor = "+cursor);
			if( cursor != null )
			{
				int cursorCount = cursor.getCount();
				//	Log.v("TabWallpaperFactory" ,"cursorCount = "+cursorCount);
				cursor.close();
				if( cursorCount > 0 )
				{
