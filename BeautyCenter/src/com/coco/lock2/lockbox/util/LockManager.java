...
public class LockManager
{
			...
			String wrapName )
	{
		if( PlatformInfo.getInstance( mContext ).isSupportViewLock() )
		{
			...
			if( FunctionConfig.isEnable_topwise_style() )
			{
				...
				editor.putString( StaticClass.USE_THIRD_PARTY_LOCK_PACKAGE_NAME , "" );
				editor.putString( StaticClass.USE_THIRD_PARTY_LOCK_CLASS_NAME , "" );
				editor.commit();
			}
			//end
	...
	public void applyThirdPartyLock(
			String packageName ,
			String className ,
			String wrapName )
	{
		...
		{
			SharedPreferences.Editor editor = PreferenceManager.getDefaultSharedPreferences( mContext ).edit();
			editor.putString( StaticClass.USE_THIRD_PARTY_LOCK_PACKAGE_NAME , packageName );
			editor.putString( StaticClass.USE_THIRD_PARTY_LOCK_CLASS_NAME , className );
			editor.commit();
			String strValue = String.format( Locale.getDefault() , "%d,%s,%s,%s" , SYS_SETTING_LOCK_VERSION , "com.third.test" , "com.third.test.testWrap" , "com.third.test.testActivity" );
			Log.d( LOG_TAG , strValue );
			android.provider.Settings.System.putString( mContext.getContentResolver() , SYS_SETTING_CURRENT_LOCK , strValue );
		}
		else
		{
			...
			editor.putString( StaticClass.USE_THIRD_PARTY_LOCK_CLASS_NAME , className );
			editor.commit();
			PubProviderHelper.addOrUpdateValue( "lock" , StaticClass.USE_LOCK_PACKAGE_NAME , "com.third.test" );
			PubProviderHelper.addOrUpdateValue( "lock" , StaticClass.USE_LOCK_CLASS_NAME , "com.third.test.testActivity" );
			PubProviderHelper.addOrUpdateValue( "lock" , StaticClass.USE_LOCK_WRAP_NAME , "com.third.test.testWrap" );
		}
	}
	
	public ComponentName queryCurrentThirdPartyLock()
	{
		SharedPreferences sharedPre = PreferenceManager.getDefaultSharedPreferences( mContext );
		String pkgName = sharedPre.getString( StaticClass.USE_THIRD_PARTY_LOCK_PACKAGE_NAME , "" );
		String className = sharedPre.getString( StaticClass.USE_THIRD_PARTY_LOCK_CLASS_NAME , "" );
		return new ComponentName( pkgName , className );
	}
