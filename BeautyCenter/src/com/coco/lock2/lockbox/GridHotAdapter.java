...
public class GridHotAdapter extends BaseAdapter
{
			...
			Set<String> pkgNameSet )
	{
		...
		//Log.i( "andy test" , "[GridHotAdapter] queryPackage before  mgr.queryShowList() !!" );
		List<LockInformation> hotList = mgr.queryShowList();
		//Log.i( "andy test" , "[GridHotAdapter] queryPackage after  mgr.queryShowList() !!" );
		if( hotList.size() == 0 )
		{
			...
			ViewGroup parent )
	{
		...
		{
			if( lockInfo.isInstalled( context ) )
			{
				lockInfo.loadDetail( context );
			}
			else
			{
				if( !new File( PathTool.getThumbFile( lockInfo.getPackageName() ) ).exists() )
				{
		...
		ImageLoader.getInstance().displayImage( "file:///" + PathTool.getThumbFile( lockInfo.getPackageName() ) , viewHolder.viewThumb , options );
	...
	public class PageTask extends AsyncTask<String , Integer , List<LockInformation>>
	{
				...
				String ... params )
		{
			...
			//Log.i( "andy test" , "[GridHotAdapter] doInBackground before  queryPackage !!" );
