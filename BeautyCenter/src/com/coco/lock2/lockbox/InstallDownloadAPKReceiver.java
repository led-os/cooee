...
public class InstallDownloadAPKReceiver extends BroadcastReceiver
{
			...
			Intent intent )
	{
		...
		{
			final String packageName = intent.getDataString().substring( 8 );// intent.getData().getSchemeSpecificPart();
			if( LOG )
			{
			...
			if( packageName != null )
			{
				...
				new Thread( new Runnable() {
					...
					public void run()
					{
						...
						precessApkInfo( context , packageName );
					}
				} ).start();
			}
			...
			if( packageName.equals( resolveInfo.activityInfo.packageName ) )
			{
				...
				mHandler.post( new Runnable() {
					...
					public void run()
					{
						...
						Log.d( TAG , "iLoongBase --- sendBroadcast " );
					}
				} );
				break;
			}
