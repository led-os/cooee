...
public class WidgetService
{
	...
	private List<ActivityInfo> queryWidgetActivityList()
	{
		...
		Intent uniIntent = new Intent( "com.uni.widget" , null );
		List<ResolveInfo> uniInfoList = pm.queryIntentActivities( uniIntent , PackageManager.GET_ACTIVITIES | PackageManager.GET_META_DATA );
		boolean isUni = false;
		for( int index = 0 ; index < themescount ; index++ )
		{
			ResolveInfo resinfo = themesinfo.get( index );
			isUni = false;
			for( ResolveInfo info : uniInfoList )
			{
				if( resinfo.activityInfo.packageName.equals( info.activityInfo.packageName ) )
				{
					isUni = true;
					break;
				}
