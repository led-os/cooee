...
public class FontPreviewActivity extends Activity
{
	...
	private boolean isApplyingFont = false;
			...
			Bundle savedInstanceState )
	{
		...
		btnApply.setOnClickListener( new View.OnClickListener() {
					...
					View v )
			{
				if( isApplyingFont )
				{
					Log.v( "isApplyingFont" , "isApplyingFont = " + isApplyingFont );
					return;
				}
				Log.v( "isApplyingFont" , "isApplyingFont = " + isApplyingFont );
				isApplyingFont = true;
				...
				{
					long size = com.coco.theme.themebox.StaticClass.getAvailableInternalMemorySize();
					Log.v( "test" , "left size = " + size );
					if( size <= ( 20 * 1024 * 1024 ) )
					{
						Toast.makeText( mContext , R.string.data_storage_is_full , Toast.LENGTH_SHORT ).show();
						return;
					}
					Toast.makeText( mContext , R.string.applyfont , Toast.LENGTH_SHORT ).show();
					new Thread( new Runnable() {
						
						public void run()
						{
							final String path = copyFile( PathTool.getAppFile( packageName ) , Information.getDisplayName() );
							final boolean isTypeface = isTypefaceFile( PathTool.getAppFile( packageName ) , path );
							//推送到UI线程
							btnApply.post( new Runnable() {
								
								public void run()
								{
									SharedPreferences pref = PreferenceManager.getDefaultSharedPreferences( FontPreviewActivity.this );
									Log.v( "test" , "path = " + path );
									if( isTypeface && path != null )
									{
										if( path.contains( pref.getString( "currentFont" , "nothing" ) ) )
										{
											Toast.makeText( mContext , R.string.apply_ok , Toast.LENGTH_SHORT ).show();
										}
										else
										{
											Intent intent = new Intent( "com.topwise.fontpath" );
											intent.putExtra( "action" , path );
											pref.edit().putString( "currentFont" , Information.getDisplayName() ).commit();
											pref.edit().putString( "applyFontExit" , "hot" ).commit();//热门换字体
											sendBroadcast( intent );
											findViewById( R.id.btnDelete ).setVisibility( View.GONE );
											Toast.makeText( mContext , R.string.apply_ok , Toast.LENGTH_SHORT ).show();
											Log.v( "test" , "sendbroadcast com.topwise.fontpath" );
										}
									}
									else
									{
										finish();
										Toast.makeText( mContext , R.string.apply_fail , Toast.LENGTH_SHORT ).show();
									}
									isApplyingFont = false;
								}
							} );
				...
				else
				{
					...
					new Thread( new Runnable() {
						...
						public void run()
						{
							final String path = copyFile( PathTool.getAppFile( packageName ) , packageName );
							//推送到UI线程
							btnApply.post( new Runnable() {
	...
	private void querryCurrentFont()
	{
			...
			String path )
	{
		try
		{
			Typeface mFontFace = Typeface.createFromFile( oldPath );
	...
	private void updateShowInfo()
	{
		...
		else
		{
			...
			String author = getString( R.string.previewThemeSize ) + Information.getApplicationSize() / 1024 + "KB     " + getString( R.string.previewThemeAuther ) + Information.getAuthor( mContext );
	...
	private void updateInforButton()
	{
		...
		if( Information.getDownloadStatus() != DownloadStatus.StatusDownloading && !currentFont.contains( Information.getDisplayName() ) )
		{
			...
			{ //文件存在时   
				...
				if( f.exists() )
				{
					if( f.isDirectory() )
					{
						...
						{
							if( child.getName().contains( pkgname ) )
							{
				...
				fs.close();
				inStream.close();
				if( fileName.length() < oldfile.length() )
				{
					path = null;
				}
				else
				{
					Log.v( "fuck" , String.valueOf( fileName.length() ) );
					Log.v( "fuck" , String.valueOf( oldfile.length() ) );
