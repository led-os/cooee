...
public class DownloadList
{
	...
	private final int REQUEST_ACTION_URL = 1301;
	private final String LOG_TAG = "DownloadTab";
	private static final String VERSION_CODE = "2";
	private static final String APP_VERSION_CODE = "39812";//需要手动填写，这个版本号跟Manifest中的versionCode一致
	...
	private class DownloadListThread extends Thread
	{
		...
		public void run()
		{
			resetListInfo( info );//清除缓存数据
			String url = SERVER_URL_TEST;
			Log.v( "downloadlist" , "SERVER_URL_TEST = " + SERVER_URL_TEST );
			...
			if( params != null )
			{
				...
				if( res != null )
				{
					...
					try
					{
						...
						Log.v( "downloadlist" , "json = " + orgjson.toString() );
						...
						if( retCode == 0 )
						{
							...
							for( Iterator iter = json.keys() ; iter.hasNext() ; )
							{
								...
								/*if( key.equals( "1" ) )
									Tools.writelogTosd( tmJson.toString() + "\n" );*/
								...
								for( Iterator mIterator = tmJson.keys() ; mIterator.hasNext() ; )
								{
									...
									else if( mKey.equals( "typeid" ) )
									{
										...
										Log.i( "downloadlist" , "mKey = " + mKey + " typeid = " + tmJson.get( mKey ).toString() );
									}
									else
									{
										...
										Log.i( "downloadlist" , "mKey = " + mKey + " jsObj.getString() = " + jsObj.getString( "cnname" ) );
										...
										{
											//Log.i( "downloadlist" , "JSONException 0 = " + e.getStackTrace() );
										...
										{
											//Log.i( "downloadlist" , "JSONException 1= " + e.getStackTrace() );
										...
										{
											//Log.i( "downloadlist" , "JSONException 2= " + e.getStackTrace() );
										...
										{
											//Log.i( "downloadlist" , "JSONException 3= " + e.getStackTrace() );
	...
	private boolean enable_Statistics_LOG = true;
	...
	private final String ACTION_OPERATE = "1304";
	private final String OPERATE_DATE = "operateDate";
	private List<ListInfo> operateInfo = new ArrayList<ListInfo>();
	private DownloadOperateThread downOperateThread = null;
	private Object syncOperateObject = new Object();
	
	public void downOperate()
	{
		synchronized( this.syncOperateObject )
		{
			if( downOperateThread == null )
			{
				downOperateThread = new DownloadOperateThread();
				downOperateThread.start();
			}
		}
	}
	
	public void stopDownloadOperate()
	{
		if( downOperateThread != null )
		{
			downOperateThread.stopRun();
			downOperateThread = null;
		}
	}
	
	private void downloadOperateFinish()
	{
		ThemeXmlParser parserXml = new ThemeXmlParser( mContext );
		HotOperateService sv = new HotOperateService( mContext );
		Intent intent = new Intent();
		sv.clearTable();
		if( parserXml.parseList( operateInfo , mContext ) )
		{
			sv.batchInsert( parserXml.getThemeList() );
		}
		saveOperateTime();
		intent.setAction( UtilsBase.ACTION_HOT_CHANGED );
		mContext.sendBroadcast( intent );
	}
	
	private void saveOperateTime()
	{
		Time curTime = new Time();
		String curDateString = curTime.format( "yyyyMMdd" );
		SharedPreferences sharedPrefer = PreferenceManager.getDefaultSharedPreferences( mContext );
		Editor edit = sharedPrefer.edit();
		edit.putString( OPERATE_DATE , curDateString );
		edit.commit();
	}
	
	public boolean isRefreshOperate()
	{
		SharedPreferences sharedPrefer = PreferenceManager.getDefaultSharedPreferences( mContext );
		String downListDate = sharedPrefer.getString( OPERATE_DATE , "" );
		Time curTime = new Time();
		String curDateString = curTime.format( "yyyyMMdd" );
		if( curDateString.equals( downListDate ) )
		{
	 ...
	 * 下载tab的线程
	 */
	private class DownloadOperateThread extends Thread
	{
		
		private volatile boolean isExit = false;
		
		public DownloadOperateThread()
		{
		}
		...
		{
			String url = SERVER_URL_TEST;
			String params = getParams( ACTION_OPERATE , true );
			boolean isSucceed = false;
			if( params != null )
			{
				CustomerHttpClient client = new CustomerHttpClient( mContext );
				...
				{
					Log.v( "OPPersonal" , "OPPersonal--------request config exception:" + res.exception.getMessage() );
				}
				else
				{
			...
			if( isSucceed )
			{
				downloadOperateFinish();
			}
			synchronized( syncOperateObject )
			{
				downOperateThread = null;
				return;
			}
