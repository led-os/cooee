...
public class ThemeInformation
{
			...
			Context context )
	{
		if( !installed )
		{
			installed = isAppInstalled( context , getPackageName() );
			return installed;
		}
		return installed;
	}
			...
			String actName )
	{
		try
		{
			...
			//Log.i( "andy" , "ThemeInformation setThumbImage thumbPath = "+thumbPath );
			...
			DownloadThemeItem item )
	{
		...
		if( downloadSize == item.getApplicationSize() )
		{
			item.setDownloadStatus( DownloadStatus.StatusFinish );
			downloadStatus = DownloadStatus.StatusFinish;
		}
		downloaded = true;
	}
