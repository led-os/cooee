...
public class OperateGridHotAdapter extends BaseAdapter
{
	
	private List<OperateInformation> appList = new ArrayList<OperateInformation>();
			...
			DownModule down )
	{
		...
		int size = preferences.getInt( "list-" + DownloadList.Operate_Type , 0 );
		if( size == 0 )
		{
			...
			String pkgName )
	{
		...
		OperateInformation info = appList.get( findIndex );
		info.reloadThumb();
	...
	public List<OperateInformation> queryPackage()
	{
		List<OperateInformation> appList = new ArrayList<OperateInformation>();
		OperateService service = new OperateService( context );
		int size = service.queryShowList( appList );
		if( size == 0 )
		{
			if( !com.coco.theme.themebox.StaticClass.isAllowDownload( context ) )
			{
	...
	{
		for( OperateInformation info : appList )
		{
			info.disposeThumb();
			info = null;
		}
			...
			ViewGroup parent )
	{
		...
		OperateInformation lockInfo = (OperateInformation)getItem( position );
		if( lockInfo.isNeedLoadDetail() )
		{
			lockInfo.loadDetail( context );
			if( lockInfo.getThumbImage() == null )
			{
				downThumb.downloadThumb( lockInfo.getPackageName() , DownloadList.Operate_Type );
			}
		}
		...
		if( FunctionConfig.isPriceVisible() )
		{
			...
			if( lockInfo.getPrice() > 0 )
			{
				...
				boolean ispay = Tools.isContentPurchased( context , DownloadList.Operate_Type , lockInfo.getPackageName() );
				if( ispay )
				{
