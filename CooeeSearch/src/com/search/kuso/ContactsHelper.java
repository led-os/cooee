...
public class ContactsHelper
{
	...
	*/
	...
	{
		Log.e( "" , "whj res isSearching " + isSearching() + " isContactsChanged " + isContactsChanged() );
		if( true == isSearching() )
		{
			return false;
		}
			...
			String search )
	{
		...
		if( mFirstNoSearchResultInput.length() > 0 )
		{
			if( search.contains( mFirstNoSearchResultInput.toString() ) )
			{
				Log.i(
						TAG ,
						"no need  to search,null!=search,mFirstNoSearchResultInput.length()=" + mFirstNoSearchResultInput.length() + "[" + mFirstNoSearchResultInput.toString() + "]" + ";searchlen=" + search
								.length() + "[" + search + "]" );
				return;
			}
			else
			{
				Log.i(
						TAG ,
						"delete  mFirstNoSearchResultInput, null!=search,mFirstNoSearchResultInput.length()=" + mFirstNoSearchResultInput.length() + "[" + mFirstNoSearchResultInput.toString() + "]" + ";searchlen=" + search
								.length() + "[" + search + "]" );
				mFirstNoSearchResultInput.delete( 0 , mFirstNoSearchResultInput.length() );
			}
		}
		...
		for( int i = 0 ; i < contactsCount ; i++ )
		{
			...
			if( true == T9MatchPinyinUnits.matchPinyinUnits( pinyinUnits , name , search , chineseKeyWord ) )
		...
		if( mSearchContacts.size() <= 0 )
		{
			if( mFirstNoSearchResultInput.length() <= 0 )
			{
				...
				Log.i(
						TAG ,
						"no search result,null!=search,mFirstNoSearchResultInput.length()=" + mFirstNoSearchResultInput.length() + "[" + mFirstNoSearchResultInput.toString() + "]" + ";searchlen=" + search
								.length() + "[" + search + "]" );
			}
			else
			{
	...
	private void registerContentObserver()
	{
		...
		if( null != mContext )
		{
			mContext.getContentResolver().registerContentObserver( ContactsContract.CommonDataKinds.Phone.CONTENT_URI , true , mContentObserver );
		}
	}
			...
			Context context )
	{
		...
		try
		{
			...
			while( cursor.moveToNext() )
			{
				...
				int contactsID = cursor.getInt( cursor.getColumnIndex( ContactsContract.CommonDataKinds.Phone.CONTACT_ID ) );
				cs = new Contacts( displayName , contactsID );
