...
final public class WallpaperUtil {
    ...
    public static void suggestWallpaperDimensions(Activity activity){
        //跨进程（wallpapaer和launcher）的sharedPreferences重新加载
        SharedPreferences sharedPreferences = activity.getSharedPreferences(
            LauncherConstants.SHARED_PREFERENCES_NAME, Context.MODE_MULTI_PROCESS);
        boolean isWallpaperFixed = sharedPreferences
            .getBoolean(LauncherConstants.SP_WALLPAPER_FIXED, true);
        int width = DeviceUtils.getScreenPixelsWidth(activity);
        int height = DeviceUtils.getRealScreenPixelsHeight(activity);
        //多屏壁纸情况
        if( !isWallpaperFixed){
            width = WALLPAPER_ROLL_FACTOR * DeviceUtils.getScreenPixelsWidth(activity);
            height = DeviceUtils.getRealScreenPixelsHeight(activity);
        }
     ...
     *
     * 除了首次开机setDefaultWallpaper，只提供给wallpaper访问。
     * wallpaper是一个独立的进程，其他进程访问，sharedPreferences值不对
    ...
    public static boolean setWallpaper(Activity context, Bitmap bitmap) {
        ...
        int height = DeviceUtils.getRealScreenPixelsHeight(context);
        //多屏壁纸情况
        if( !LauncherPreference.getWallpaperFixed()){
            width = WALLPAPER_ROLL_FACTOR * DeviceUtils.getScreenPixelsWidth(context);
            height = DeviceUtils.getRealScreenPixelsHeight(context);
        }
