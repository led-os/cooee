...
public class PagedViewWidget extends LinearLayout
{
			...
			int defStyle )
	{
		...
		mDimensionsFormatString = LauncherDefaultConfig.getString( R.string.widget_dims_format );
	...
	protected void onFinishInflate()
	{
		...
		if( name != null )
		{
			name.setTextSize( TypedValue.COMPLEX_UNIT_SP , grid.getIconTextSize() );
		}
		TextView dims = (TextView)findViewById( R.id.widget_dims );
		if( dims != null )
		{
			dims.setTextSize( TypedValue.COMPLEX_UNIT_SP , grid.getIconTextSize() );
		}
	}
	
	public static void setDeletePreviewsWhenDetachedFromWindow(
			boolean value )
	{
		sDeletePreviewsWhenDetachedFromWindow = value;
	}
	
	public static void setRecyclePreviewsWhenDetachedFromWindow(
			boolean value )
	{
	...
	protected void onDetachedFromWindow()
	{
		...
		if( sDeletePreviewsWhenDetachedFromWindow )
		{
			...
			if( image != null )
			{
				...
				if( sRecyclePreviewsWhenDetachedFromWindow && mInfo != null && preview != null && preview.getBitmap() != null )
				{
			...
			WidgetPreviewLoader loader )
	{
		...
		if( dims != null )
		{
			int hSpan = Math.min( cellSpan[0] , (int)grid.getNumColumns() );
			int vSpan = Math.min( cellSpan[1] , (int)grid.getNumRows() );
	...
	public void setShortPressListener(
			ShortPressListener listener )
	{
