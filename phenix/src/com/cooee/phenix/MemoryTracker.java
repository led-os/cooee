...
public class MemoryTracker extends Service
{
	...
	public static final String ACTION_START_TRACKING = "com.cooee.phenix.action.START_TRACKING";
	...
	public static void startTrackingMe(
			Context context ,
			String name )
	{
		context.startService( new Intent( context , MemoryTracker.class ).setAction( MemoryTracker.ACTION_START_TRACKING ).putExtra( "pid" , android.os.Process.myPid() ).putExtra( "name" , name ) );
	}
	
	public ProcessMemInfo getMemInfo(
			int pid )
	{
	...
	public void startTrackingProcess(
			int pid ,
			String name ,
			long start )
	{
	...
	void update()
	{
		synchronized( mLock )
		{
			...
			for( int i = 0 ; i < dinfos.length ; i++ )
			{
				...
				{
					if( LauncherDefaultConfig.SWITCH_ENABLE_DEBUG )
						Log.v( TAG , StringUtils.concat( "update: pid " , pid , " has pss=0, it probably died" ) );
					mData.remove( pid );
				}
			}
	...
	public void onCreate()
	{
		...
		for( ActivityManager.RunningServiceInfo svc : svcs )
		{
			...
			{
				if( LauncherDefaultConfig.SWITCH_ENABLE_DEBUG )
					Log.v( TAG , StringUtils.concat( "discovered running service: " , svc.process , " (" , svc.pid , ")" ) );
		...
		for( ActivityManager.RunningAppProcessInfo proc : procs )
		{
			...
			{
				if( LauncherDefaultConfig.SWITCH_ENABLE_DEBUG )
					Log.v( TAG , StringUtils.concat( "discovered other running process: " , pname , " (" , proc.pid , ")" ) );
	...
	{
		if( LauncherDefaultConfig.SWITCH_ENABLE_DEBUG )
			Log.v( TAG , StringUtils.concat( "Received start id " , startId , ": " , intent.toUri( 0 ) ) );
		if( intent != null )
		{
