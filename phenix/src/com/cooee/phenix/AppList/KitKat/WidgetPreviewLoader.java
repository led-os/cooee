...
public class WidgetPreviewLoader
{
			...
			Context context )
	{
		...
		mAppIconSize = grid.getIconWidthSizePx();
	...
	public static class CacheDb extends SQLiteOpenHelper
	{
		...
		{
			database.execSQL( StringUtils.concat(
				...
				int newVersion )
		{
			if( oldVersion != newVersion )
			{
				...
				db.execSQL( StringUtils.concat( "DELETE FROM " , TABLE_NAME ) );
			}
		}
	}
	...
	private String getObjectPackage(
			Object o )
	{
		if( o instanceof AppWidgetProviderInfo )
		{
	...
	public static void removePackageFromDb(
			...
			final String packageName )
	{
		...
		new AsyncTask<Void , Void , Void>() {
					...
					Void ... args )
			{
				...
				db.delete( CacheDb.TABLE_NAME , StringUtils.concat( CacheDb.COLUMN_NAME , " LIKE ? OR " , CacheDb.COLUMN_NAME , " LIKE ?" ) , // SELECT query
						new String[]{ StringUtils.concat( WIDGET_PREFIX , packageName , "/%" ) , StringUtils.concat( SHORTCUT_PREFIX , packageName , "/%" ) } // args to SELECT query
				);
				synchronized( sInvalidPackages )
				{
	...
	public static void removeItemFromDb(
			final CacheDb cacheDb ,
			final String objectName )
	{
		new AsyncTask<Void , Void , Void>() {
			...
			{
				SQLiteDatabase db = cacheDb.getWritableDatabase();
				db.delete( CacheDb.TABLE_NAME , StringUtils.concat( CacheDb.COLUMN_NAME , " = ? " ) , // SELECT query
	...
	private Bitmap readFromDb(
			String name ,
			Bitmap b )
	{
		if( mCachedSelectQuery == null )
		{
			mCachedSelectQuery = StringUtils.concat( CacheDb.COLUMN_NAME , " = ? AND " , CacheDb.COLUMN_SIZE , " = ?" );
		}
			...
			Bitmap preview )
	{
		...
		{
			//zhujieping del，因为tab栏会隐藏或显示，小组件内容的大小会发生变化，这里的大小是会不一致的
			...
			int[] preScaledWidthOut )
	{
		...
		if( previewImage != 0 )
		{
			...
			if( drawable == null )
			{
				...
				{
					Log.w( TAG , StringUtils.concat( "Can't load widget preview drawable 0x" , Integer.toHexString( previewImage ) , " for provider:" , provider.toString() ) );
				}
			}
		}
		int previewWidth;
		...
		else
		{
			...
			try
			{
				...
				if( iconId > 0 )
				{
					...
					//					icon = mIconCache.getFullResIcon( packageName , iconId );//cheyingkun del
					icon = mIconCache.getFullResIcon( packageName , iconId , Utilities.sIconTextureWidth , Utilities.sIconTextureHeight );//cheyingkun add
					//cheyingkun end
				}
				if( icon != null )
				{
		...
		// zhujieping@2015/04/27 ADD START，保证小组件显示在区域内
		if( scale * previewHeight > mPreviewBitmapHeight )
		{
			scale = mPreviewBitmapHeight / (float)previewHeight;
		}
			...
			Bitmap preview )
	{
		...
		// Render the icon
		//cheyingkun start	//获取高分辨率代码整理和注释。
		//		Drawable icon = mIconCache.getFullResIcon( info );//cheyingkun del
		Drawable icon = mIconCache.getFullResIcon( info.activityInfo , Utilities.sIconTextureWidth , Utilities.sIconTextureHeight );//cheyingkun add
		//cheyingkun end
		...
		// zhujieping@2015/05/05 ADD START,防止1*1的widget后面放大的图标过大，被截掉
		if( scaledIconWidth > maxHeight )
		{
			scaledIconWidth = maxHeight;
		}
		// zhujieping@2015/05/05 ADD END
		...
		{
			// zhujieping@2015/05/05 UPD START，打开开关switch_enable_show_appbar_in_applist时，小组件区域会有变化，会存在大小变化的情况
			//throw new RuntimeException( "Improperly sized bitmap passed as argument" );
			Bitmap tmp = preview;
			preview = Bitmap.createBitmap( maxWidth , maxHeight , Config.ARGB_8888 );
			tmp.recycle();
			// zhujieping@2015/05/05 UPD END
		}
