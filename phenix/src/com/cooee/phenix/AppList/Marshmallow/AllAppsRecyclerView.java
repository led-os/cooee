...
import com.cooee.phenix.BaseRecyclerView;
import com.cooee.phenix.BaseRecyclerViewFastScrollBar;
import com.cooee.phenix.DeviceProfile;
import com.cooee.phenix.R;
import com.cooee.phenix.Stats;
...
public class AllAppsRecyclerView extends BaseRecyclerView implements Stats.LaunchSourceProvider
{
			...
			int defStyleAttr )
	{
		...
		if( isScrollBarVisible && mScrollbar != null )//zhujieping add  //拓展配置项“config_applist_style”，添加可配置项2。2是7.0的主菜单样式。
			mScrollbar.setDetachThumbOnFastScroll();
		mEmptySearchBackgroundTopOffset = LauncherDefaultConfig.getDimensionPixelSize( R.dimen.all_apps_empty_search_bg_top_offset );
	}
			...
			int numAppsPerRow )
	{
		...
		int approxRows = (int)Math.ceil( grid.getAvailableHeightPx() / grid.getIconHeightSizePx() );
	...
	public void scrollToTop()
	{
		...
		if( isScrollBarVisible && mScrollbar != null && mScrollbar.isThumbDetached() )//zhujieping add  //拓展配置项“config_applist_style”，添加可配置项2。2是7.0的主菜单样式。
		{
			...
			Canvas c )
	{
		...
		{
			c.clipRect( mBackgroundPadding.left , mBackgroundPadding.top , getWidth() - mBackgroundPadding.right , getHeight() - mBackgroundPadding.bottom );
	...
	protected void onFinishInflate()
	{
		...
		// Bind event handlers
		if( mScrollbar != null )//zhujieping add  //拓展配置项“config_applist_style”，添加可配置项2。2是7.0的主菜单样式。
			mScrollbar.setDetachThumbOnFastScroll();
		addOnItemTouchListener( this );
	}
	
	@Override
	public void fillInLaunchSourceData(
			Bundle sourceData )
	{
	 ...
	 */
	@Override
	public String scrollToPositionAtProgress(
			float touchFraction )
	{
			...
			int dy )
	{
		...
		if( !isScrollBarVisible || mScrollbar == null )
			return;
		//zhujieping add end
		...
		// padding)
		int scrollY = getPaddingTop() + ( mScrollPosState.rowIndex * mScrollPosState.rowHeight ) - mScrollPosState.rowTopOffset;
	...
	Runnable mSmoothSnapNextFrameRunnable = new Runnable() {
		...
		public void run()
		{
			...
			else
			{
				...
				final ViewHolder vh = findViewHolderForPosition( mPrevFastScrollFocusedPosition );
				if( vh != null && vh.itemView instanceof BaseRecyclerViewFastScrollBar.FastScrollFocusableView && mLastFastScrollFocusedView != vh.itemView )
				{
					mLastFastScrollFocusedView = (BaseRecyclerViewFastScrollBar.FastScrollFocusableView)vh.itemView;
					mLastFastScrollFocusedView.setFastScrollFocused( true , true );
				}
	 ...
	 */
	private void smoothSnapToPosition(
			final int position ,
			ScrollPositionState scrollPosState )
	{
		...
		{
			ViewCompat.postOnAnimation( this , mSmoothSnapNextFrameRunnable );
		}
	 ...
	 */
	protected void getCurScrollState(
	 ...
	 */
	private int getScrollAtPosition(
			int position ,
			int rowHeight )
	{
