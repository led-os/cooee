...
import com.cooee.phenix.AppList.Nougat.AllAppsNougatGridAdapter;
import com.cooee.phenix.AppList.Nougat.FirstLetterMapper;
import com.cooee.phenix.AppList.Nougat.favorites.FavoritesAppData;
import com.cooee.phenix.compat.AlphabeticIndexCompat;
import com.cooee.phenix.compat.UserHandleCompat;
import com.cooee.phenix.config.defaultConfig.LauncherDefaultConfig;
import com.cooee.phenix.data.AppInfo;
import com.cooee.phenix.model.AppNameComparator;
 ...
 */
public class AlphabeticalAppsList implements IOnThemeChanged //zhujieping add,换主题不重启
{
	...
	public static class AdapterItem
	{
		...
		public static AdapterItem asDivider(
				int pos )
		{
			AdapterItem item = new AdapterItem();
		...
		public static AdapterItem asMarketSearch(
				int pos )
		{
			AdapterItem item = new AdapterItem();
	...
	private final HashMap<ComponentKey , AppInfo> mComponentToAppMap = new HashMap<ComponentKey , AppInfo>();
	// The set of filtered apps with the current filter
	private List<AppInfo> mFilteredApps = new ArrayList<AppInfo>();
	// The current set of adapter items
	private List<AdapterItem> mAdapterItems = new ArrayList<AdapterItem>();
	...
	private List<FastScrollSectionInfo> mFastScrollerSections = new ArrayList<FastScrollSectionInfo>();
	// The set of predicted app component names
	private List<ComponentKey> mPredictedAppComponents = new ArrayList<ComponentKey>();
	// The set of predicted apps resolved from the component names and the current set of apps
	private List<AppInfo> mPredictedApps = new ArrayList<AppInfo>();
	// The of ordered component names as a result of a search query
	private ArrayList<ComponentKey> mSearchResults;
	private HashMap<CharSequence , String> mCachedSectionNames = new HashMap<CharSequence , String>();
	...
	private List<ComponentKey> mFavoritesAppComponents = new ArrayList<ComponentKey>();
	private List<AppInfo> mFavoritesApps = new ArrayList<AppInfo>();
	//zhujieping add end
			...
			MergeAlgorithm mergeAlgorithm )
	{
		...
		{
			FavoritesAppData.setDefaultFavoritesNum( numAppsPerRow );
		}
		//zhujieping add end
	}
	 ...
	 */
	public void setOrderedFilter(
			ArrayList<ComponentKey> f )
	{
		if( mSearchResults != f )
		{
			mSearchResults = f;
	...
	private void onAppsUpdated()
	{
		...
		boolean localeRequiresSectionSorting = false;
		if( LauncherDefaultConfig.CONFIG_APPLIST_STYLE == LauncherDefaultConfig.APPLIST_SYTLE_NOUGAT )//zhujieping add //需求：拓展配置项“config_applist_style”，添加可配置项2。2是7.0的主菜单样式。
		{
			localeRequiresSectionSorting = true;
		}
		else
		//zhujieping add end
		{
			localeRequiresSectionSorting = curLocale.equals( Locale.SIMPLIFIED_CHINESE );
		}
		if( localeRequiresSectionSorting )
		{
			// Compute the section headers.  We use a TreeMap with the section name comparator to
			// ensure that the sections are ordered when we iterate over it later
			sectionMap = new TreeMap<String , ArrayList<AppInfo>>( mAppNameComparator.getSectionNameComparator() );
			for( AppInfo info : mApps )
			{
				...
				if( sectionApps == null )
				{
					sectionApps = new ArrayList<AppInfo>();
					sectionMap.put( sectionName , sectionApps );
				}
				sectionApps.add( info );
			}
			// Add each of the section apps to the list in order
			List<AppInfo> allApps = new ArrayList<AppInfo>( mApps.size() );
	...
	private void updateAdapterItems()
	{
		...
		if( DEBUG_PREDICTIONS )
		{
			...
			{
				mPredictedAppComponents.add( new ComponentKey( mApps.get( 0 ).getComponentName() , UserHandleCompat.myUserHandle() ) );
				mPredictedAppComponents.add( new ComponentKey( mApps.get( 0 ).getComponentName() , UserHandleCompat.myUserHandle() ) );
				mPredictedAppComponents.add( new ComponentKey( mApps.get( 0 ).getComponentName() , UserHandleCompat.myUserHandle() ) );
				mPredictedAppComponents.add( new ComponentKey( mApps.get( 0 ).getComponentName() , UserHandleCompat.myUserHandle() ) );
			}
		...
		if( mFavoritesAppComponents != null && !mFavoritesAppComponents.isEmpty() && !hasFilter() )
		{
			for( ComponentKey ck : mFavoritesAppComponents )
			{
				AppInfo info = mComponentToAppMap.get( ck );
				if( info != null )
				{
					mFavoritesApps.add( info );
				}
				// Stop at the number of predicted apps
				if( mFavoritesApps.size() == mNumPredictedAppsPerRow )
				{
					break;
				}
			}
			if( !mFavoritesApps.isEmpty() )
			{
				// Add a section for the predictions
				lastSectionInfo = new SectionInfo();
				lastFastScrollerSectionInfo = new FastScrollSectionInfo( "" );
				...
				mAdapterItems.add( sectionItem );
				// Add the predicted app items
				for( AppInfo info : mFavoritesApps )
				{
					AdapterItem appItem = AdapterItem.asApp( position++ , lastSectionInfo , AllAppsNougatGridAdapter.FAVORITES_TAG , lastSectionInfo.numApps++ , info , appIndex++ );
					if( lastSectionInfo.firstAppItem == null )
					{
						lastSectionInfo.firstAppItem = appItem;
						lastFastScrollerSectionInfo.fastScrollToItem = appItem;
	 ...
	 */
	private String getAndUpdateCachedSectionName(
			CharSequence title )
	{
		...
		if( sectionName == null )
		{
			...
			if( LauncherDefaultConfig.CONFIG_APPLIST_STYLE == LauncherDefaultConfig.APPLIST_SYTLE_NOUGAT )//zhujieping add //需求：拓展配置项“config_applist_style”，添加可配置项2。2是7.0的主菜单样式。
			{
				sectionName = FirstLetterMapper.getUpperCaseFirstLetter( mLauncher , title.toString() );
			}
			else
			//zhujieping add end
			{
			...
			Object arg1 )
	{
		...
		Set<ComponentKey> keys = mComponentToAppMap.keySet();
		for( ComponentKey key : keys )
		{
			mComponentToAppMap.get( key ).onThemeChanged( arg0 , arg1 );
		}
	}
	
	//zhujieping add start//需求：拓展配置项“config_applist_style”，添加可配置项2。2是7.0的主菜单样式。
	public void setFavoritesApps(
			ArrayList<ComponentKey> apps )
	{
		mFavoritesAppComponents.clear();
		mFavoritesAppComponents.addAll( apps );
