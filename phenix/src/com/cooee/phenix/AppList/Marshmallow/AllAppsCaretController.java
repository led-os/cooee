...
public class AllAppsCaretController {
    ...
    public AllAppsCaretController(CaretDrawable caret, Launcher launcher) {
		...
		//zhujieping add start,偶现动画结束时mCaretAnimator.getAnimatedValue()值与设置的值不相等，在动画结束时添加保护
		mCaretAnimator.addListener( new AnimatorListener() {
					...
					Animator animation )
			{
				...
				if( !isCancel )
				{
					Float endValue = (Float)mCaretAnimator.getAnimatedValue();
					if( Float.compare( mLastCaretProgress , endValue ) != 0 )
					{
						mCaretDrawable.setCaretProgress( mLastCaretProgress );
					}
				}
			}
