...
public class VerticalPullDetector {
	...
	private int upOffset = 0;//zhujieping add //拓展配置项“config_applist_in_and_out_anim_style”，添加可配置项2。2是仿S8进入主菜单的动画。
    ...
    public VerticalPullDetector(Context context) {
		...
		{
			upOffset = -LauncherDefaultConfig.getDimensionPixelSize( R.dimen.applist_in_and_out_anim_s8_style_move_up_offset );
		}
		//zhujieping add end
    }
    ...
    private boolean shouldScrollStart() {
        ...
        if (((mScrollConditions & DIRECTION_DOWN) > 0 && mDisplacementY > 0) ||
				( ( mScrollConditions & DIRECTION_UP ) > 0 && mDisplacementY < upOffset ) )//zhujieping //拓展配置项“config_applist_in_and_out_anim_style”，添加可配置项2。2是仿S8进入主菜单的动画。
		{
            return true;
        }
        return false;
