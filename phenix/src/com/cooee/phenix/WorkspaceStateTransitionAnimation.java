...
class TransitionStates {
    ...
    public TransitionStates(final Workspace.State fromState, final Workspace.State toState) {
		...
		oldStateIsNormalHidden = ( fromState == Workspace.State.SMALL );
		oldStateIsOverviewHidden = false;//(fromState == Workspace.State.OVERVIEW_HIDDEN);
		...
		stateIsNormalHidden = ( toState == Workspace.State.SMALL );
		stateIsOverviewHidden = false;// ( toState == Workspace.State.OVERVIEW_HIDDEN );
...
public class WorkspaceStateTransitionAnimation {
    ...
    public WorkspaceStateTransitionAnimation(Launcher launcher, Workspace workspace) {
		...
		mAllAppsTransitionTime = LauncherDefaultConfig.getInt( R.integer.config_allAppsTransitionTime );
		mOverviewTransitionTime = LauncherDefaultConfig.getInt( R.integer.config_overviewTransitionTime );
		mOverlayTransitionTime = LauncherDefaultConfig.getInt( R.integer.config_overlayTransitionTime );
        mSpringLoadedTransitionTime = mOverlayTransitionTime / 2;
		mSpringLoadedShrinkFactor = mLauncher.getDeviceProfile().getWorkspaceSpringLoadShrinkFactor();
        mOverviewModeShrinkFactor =
				LauncherDefaultConfig.getInt( R.integer.config_workspaceOverviewShrinkPercentage ) / 100f;
		mWorkspaceScrimAlpha = LauncherDefaultConfig.getInt( R.integer.config_workspaceScrimAlpha ) / 100f;
		mWorkspaceFadeInAdjacentScreens = false;
    }

    public void snapToPageFromOverView(int whichPage) {
		mWorkspace.snapToPage( whichPage , mOverviewTransitionTime , true );
    }
    ...
    private int getAnimationDuration(TransitionStates states) {
		...
		/*	else if( mLauncher.getState() == Launcher.State.WORKSPACE_SPRING_LOADED
    ...
    private void animateWorkspace(final TransitionStates states, final boolean animated,
        ...
        if (states.stateIsOverview || states.stateIsOverviewHidden) {
			...
			finalWorkspaceTranslationY = 0;//zhujieping //这边改成跟workspace中getChangeStateAnimation中finalWorkspaceTranslationY一致
		}

        final int childCount = mWorkspace.getChildCount();
		final int customPageCount = mWorkspace.getFunctionPagesInNormalPageLeftNum();

        mNewScale = 1.0f;

        if (states.oldStateIsOverview) {
			mWorkspace.disableFreeScroll( -1 );
		...
		int toPage = mWorkspace.getPageNearestToCenterOfScreen( mWorkspace.getScrollX() );
		...
		final ViewGroup overviewPanel = (ViewGroup)mLauncher.getOverviewPanel();
		final View qsbContainer = mLauncher.getSearchBar();
        ...
        if (animated) {
			...
			layerViews.put( mLauncher.getHotseat() , LauncherStateTransitionAnimation.BUILD_AND_SET_LAYER );
			layerViews.put( mWorkspace.getPageIndicator() , LauncherStateTransitionAnimation.BUILD_AND_SET_LAYER );
			...
			mWorkspace.updateFunctionPagesVisibility();
	...
	static class AlphaUpdateListener extends AnimatorListenerAdapter implements ValueAnimator.AnimatorUpdateListener
	{
