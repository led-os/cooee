/* 文件名: PubContentProvider.java 2014年8月26日
 * 
 * 描述:桌面的ContentProvider数据库类,存储主题,壁纸,锁屏等的信息状态等
 * 
 * 作者: cooee */
...
public class PubContentProvider extends ContentProvider
{
	...
	public final static String LAUNCHER_AUTHORITY = "com.cooee.phenix.center.pub.provider";
			...
			String[] selectionArgs )
	{
		...
		if( routeMatch != null )
		{
			...
			else if( routeMatch.type.equals( "item" ) )
			{
				...
				String where = StringUtils.concat( "_id = " , id );
				if( selection != null && !"".equals( selection ) )
				{
					where = StringUtils.concat( selection , " and " , where );
				}
			...
			{
				throw new IllegalArgumentException( StringUtils.concat( "Unkwon type:" , routeMatch.type , " for Uri:" , uri.toString() ) );
			}
		}
		else
		{
			throw new IllegalArgumentException( StringUtils.concat( "Unkwon Uri:" , uri.toString() ) );
		}
	}
			...
			Uri uri )
	{
		...
		if( routeMatch != null )
		{
			if( routeMatch.type.equals( "dir" ) )
			{
				return StringUtils.concat( "vnd.android.cursor.dir/" , routeMatch.table );
			}
			else if( routeMatch.type.equals( "item" ) )
			{
				return StringUtils.concat( "vnd.android.cursor.item/" , routeMatch.table );
			}
			else
			{
				throw new IllegalArgumentException( StringUtils.concat( "Unkwon type:" , routeMatch.type , "-Uri:" , uri.toString() ) );
			}
		}
		else
		{
