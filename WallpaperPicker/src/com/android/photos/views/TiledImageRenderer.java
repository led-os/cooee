...
public class TiledImageRenderer
{
	...
	public static int suggestedTileSize(
			Context context )
	{
		...
		synchronized( mQueueLock )
		{
			...
			for( int i = 0 ; i < n ; i++ )
			{
				...
				if( level < fromLevel || level >= endLevel || !range[level - fromLevel].contains( tile.mX , tile.mY ) )
				{
					...
					{
						long key = mActiveTiles.keyAt( i );
						mActiveTiles.remove( key );
					}
					else
					{
	...
	private static long makeTileKey(
			int x ,
			int y ,
			int level )
	{
