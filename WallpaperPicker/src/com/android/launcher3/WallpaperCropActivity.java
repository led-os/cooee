...
public class WallpaperCropActivity extends Activity
{
	...
	public final String LAUNCHER_CONTENT_PROVIDER_AUTHORITY_KEY = "LauncherContentProviderAuthority";
	public final String LAUNCHER_CONTENT_PROVIDER_AUTHORITY_VAULE_DEFAULT = "com.cooee.phenix.center.pub.provider";
	public final String CUSTOM_WALLPAPER_PATH_KEY = "config_custom_wallpapers_path_launcher_3";
	protected String mCustomWallpapersPath = null;
	//xiatian add end
	...
	public final String WALLPAPER_PREVIEW_STYLE_KEY = "config_custom_wallpapers_preview_style_launcher_3";
	public final static int WALLPAPER_PREVIEW_STYLE_ENABLE_DRAG = 0;//壁纸预览图样式，可拖拽居中显示。
	public final static int WALLPAPER_PREVIEW_STYLE_DISABLE_DRAG = 1;//壁纸预览图样式，不可拖拽居中显示。
	protected static int mWallpapersPreviewStyle = WALLPAPER_PREVIEW_STYLE_ENABLE_DRAG;//壁纸预览图显示样式。0为可拖动且居中显示 ，1为不可拖动且居中显示。默认为0。 
	//fulijuan add end
			...
			Bundle savedInstanceState )
	{
		...
		thisActivity = this;
		mCustomWallpapersPath = getCustomWallpapersPath();//xiatian add	//添加配置项“config_custom_wallpapers_path_launcher_3”，launcher3壁纸选择界面的壁纸路径的本地化配置。默认为空。 
		mWallpapersPreviewStyle =Integer.parseInt(getWallpapersPreviewStyle()) ;//fulijuan add  //添加配置项“config_launcher_3_wallpapers_preview_style”，launcher3壁纸选择界面的壁纸预览图显示样式。0为可拖动且居中显示 ，1为不可拖动且居中显示。默认为0。 
		init();
		if( !enableRotation() )
		{
	...
	// horizontally
	private static float wallpaperTravelToScreenWidthRatio(
			int width ,
			int height )
	{
	...
	static protected Point getDefaultWallpaperSize(
			Resources res ,
			WindowManager windowManager )
	{
		if( sDefaultWallpaperSize == null )
		{
			...
			{
				getCurrentSizeRange( thisActivity , minDims , maxDims );
			}
			// gaominghui@2016/12/14 ADD END
	...
	// gaominghui@2016/12/14 ADD START
	public static void getCurrentSizeRange(
			Activity activity ,
	...
	private static boolean isScreenLarge(
			Resources res )
	{
			...
			final boolean finishActivityWhenDone )
	{
		...
		{
			ltr = ( mCropView.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL );
		}
		// gaominghui@2016/12/14 ADD END
	...
	protected static class BitmapCropTask extends AsyncTask<Void , Void , Boolean>
	{
		...
		public void setOnBitmapCropped(
				OnBitmapCroppedHandler handler )
		{
	...
	protected void updateWallpaperDimensions(
			int width ,
			int height )
	{
	...
	protected static String getFileExtension(
	...
	//xiatian add start	//添加配置项“config_custom_wallpapers_path_launcher_3”，launcher3壁纸选择界面的壁纸路径的本地化配置。默认为空。
	private String getCustomWallpapersPath() 
	{
		//fulijuan start //添加配置项“config_launcher_3_wallpapers_preview_style”，launcher3壁纸选择界面的壁纸预览图显示样式。0为可拖动且居中显示 ，1为不可拖动且居中显示。默认为0。 
		//fulijuan del start
//		String mLauncherContentProviderAuthorityVaule = getMetaDataStringFromActivity( LAUNCHER_CONTENT_PROVIDER_AUTHORITY_KEY , LAUNCHER_CONTENT_PROVIDER_AUTHORITY_VAULE_DEFAULT );
//		ContentResolver resolver = getContentResolver();
//		Uri uri = Uri.parse( "content://" + mLauncherContentProviderAuthorityVaule + "/config"/* LauncherContentProvider的表config */);
//		String where = " propertyName=? ";
//		String[] args = new String[]{ CUSTOM_WALLPAPER_PATH_KEY };
...
//		return result;
		//fulijuan del end
		return getValueFromLauncherContentProvider(CUSTOM_WALLPAPER_PATH_KEY);//fulijuan add
		//fulijuan end
	}
	
	private String getMetaDataStringFromActivity(
			String key ,
			String defValue )
	{
	 ...
	 * @return
	 */
	private String getWallpapersPreviewStyle() {
		return getValueFromLauncherContentProvider(WALLPAPER_PREVIEW_STYLE_KEY);
	}
	 ...
	 * @return
	 */
	private String getValueFromLauncherContentProvider(String key) {
		String mLauncherContentProviderAuthorityVaule = getMetaDataStringFromActivity( LAUNCHER_CONTENT_PROVIDER_AUTHORITY_KEY , LAUNCHER_CONTENT_PROVIDER_AUTHORITY_VAULE_DEFAULT );
		ContentResolver resolver = getContentResolver();
		Uri uri = Uri.parse( "content://" + mLauncherContentProviderAuthorityVaule + "/config"/* LauncherContentProvider的表config */);
		String where = " propertyName=? ";
